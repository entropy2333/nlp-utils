...
def get_full_or_width_characters(
    mode="full", include_a_z=True, include_A_Z=True, include_0_9=True, include_punctuation=True, index=False
):
    index_list = []
    if include_a_z:
...
def convert_full_and_half_width(
    ...
    """
    full_width_characters = get_full_or_width_characters(
        mode="full",
    ...
    half_width_characters = get_full_or_width_characters(
        mode="half",
        include_a_z=include_a_z,
        include_A_Z=include_A_Z,
        include_0_9=include_0_9,
    ...
    assert len(full_width_characters) == len(half_width_characters), (
        len(full_width_characters),
...
def text2ner_label_with_exactly_match(
    transcripts: List[str], exactly_entities_label: List[List[str]], markup_type: str = "bio"
) -> List[List[str]]:
    """
