"""
mainly copied from https://github.com/Shivanandroy/simpleT5/blob/main/simplet5/simplet5.py
"""
...
class SimpleT5:
    ...
    def from_pretrained(self, model_type="t5", model_name="t5-base", special_tokens=[], **kwargs) -> None:
        ...
        """
        if model_type == "t5":
            self.tokenizer = T5Tokenizer.from_pretrained(f"{model_name}", add_special_tokens=special_tokens, **kwargs)
    ...
    def load_model(self,
        ...
        if model_type == "t5":
            ...
            self.tokenizer = T5Tokenizer.from_pretrained(f"{model_dir}", add_special_tokens=special_tokens, **kwargs)
        elif model_type == "mt5":
            self.model = MT5ForConditionalGeneration.from_pretrained(f"{model_dir}")
            self.tokenizer = MT5Tokenizer.from_pretrained(f"{model_dir}", add_special_tokens=special_tokens, **kwargs)
        elif model_type == "byt5":
            self.model = T5ForConditionalGeneration.from_pretrained(f"{model_dir}")
            self.tokenizer = ByT5Tokenizer.from_pretrained(f"{model_dir}", add_special_tokens=special_tokens, **kwargs)
        elif model_type == "bart":
            self.model = BartForConditionalGeneration.from_pretrained(f"{model_dir}")
            self.tokenizer = BartTokenizer.from_pretrained(f"{model_dir}", add_special_tokens=special_tokens, **kwargs)
        elif model_type == "cpt":
