"""
Official Repository: https://github.com/zjunlp/HVPNeT
"""
import os
...
class HMNeTREModel(nn.Module):
    ...
    def get_visual_prompt(self, images, aux_imgs):
        ...
        aux_prompt_guids = [
            torch.cat(aux_prompt_guid, dim=1).view(bsz, self.args.prompt_len, -1)
            for aux_prompt_guid in aux_prompt_guids
        ]  # 3 x [bsz, 4, 3840]

        prompt_guids = self.encoder_conv(prompt_guids)  # bsz, 4, 4*2*768
        aux_prompt_guids = [
            self.encoder_conv(aux_prompt_guid) for aux_prompt_guid in aux_prompt_guids
        ]  # 3 x [bsz, 4, 4*2*768]
        split_prompt_guids = prompt_guids.split(768 * 2, dim=-1)  # 4 x [bsz, 4, 768*2]
        split_aux_prompt_guids = [
            aux_prompt_guid.split(768 * 2, dim=-1) for aux_prompt_guid in aux_prompt_guids
        ]  # 3x [4 x [bsz, 4, 768*2]]
        ...
        for idx in range(12):  # 12
            ...
            for split_aux_prompt_guid in split_aux_prompt_guids:
                ...
                for i in range(4):
                    aux_key_val = aux_key_val + torch.einsum(
                        "bg,blh->blh", aux_prompt_gate[:, i].view(-1, 1), split_aux_prompt_guid[i]
                    )
            ...
            key, value = (
                key_val[0].reshape(bsz, 12, -1, 64).contiguous(),
                key_val[1].reshape(bsz, 12, -1, 64).contiguous(),
            )  # bsz, 12, 4, 64
            temp_dict = (key, value)
...
class HMNeTNERModel(nn.Module):
    ...
    def forward(
        self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None, images=None, aux_imgs=None
    ):
        if self.args.use_prompt:
            prompt_guids = self.get_visual_prompt(images, aux_imgs)
            prompt_guids_length = prompt_guids[0][0].shape[2]
